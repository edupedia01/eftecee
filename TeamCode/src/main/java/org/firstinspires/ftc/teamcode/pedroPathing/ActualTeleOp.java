...
//        robot.intake.on().schedule();
    ...
    public void loop() {
        ...
        // ---- INTAKE SI BLOCKER----
        if (gamepad1.dpadUpWasPressed()) robot.intake.on().schedule();
        if (gamepad1.dpadUpWasReleased()) robot.intake.off().schedule();
        if (gamepad1.dpadDownWasPressed()) robot.intake.reverse().schedule();
        if (gamepad1.dpadDownWasPressed()) robot.intake.off().schedule();
